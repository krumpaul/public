...
function homepage()
	while true do
		...
		local maxVal = turbine.fluidTank().input().maxAmount()
		...
		local maxVal = turbine.fluidTank().output().maxAmount()
		local minVal = math.floor(turbine.fluidTank().output().amount())
		...
		local maxVal = turbine.battery().capacity()
...
function update()
	...
	shell.run("pastebin get XmsSWZEi current_version.txt")
...
function setup_wizard()
	...
	draw_text_term(1, 4, "type 'default' for  'BiggerReactors_Turbine_0'", colors.gray, colors.black)

	term.setTextColor(colors.white) 
	term.setCursorPos(1,5)
	name = read()

	if name == "default" then name = "BiggerReactors_Turbine_0" end
	auto_string = false
	on = 0
	off = 99
